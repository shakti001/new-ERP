    ...
    def create(self , validated_data):
        ...
        if not globalSettings.LITE_REGISTRATION:
            if globalSettings.VERIFY_MOBILE:
                ...
                print mobile
                try:
                    url = globalSettings.SMS_API_PREFIX + 'mobiles=%s&message=%s'%(mobile , 'Hi, %s is the OTP to verify your mobile number for Happy Pockets' %(reg.mobileOTP))
                except:
                    url = globalSettings.SMS_API_PREFIX + 'number=%s&message=%s'%(mobile , 'Hi, %s is the OTP to verify your mobile number for Happy Pockets' %(reg.mobileOTP))
                requests.get(url)
        else:
            mobile = reg.mobile
