    ...
    for (var i = 0; i < $scope.data.tableData.length; i++) {
      if ($scope.data.tableData[i].pk == parseInt(target)) {
        if (action == 'edit') {
          ...
          $http({
            method: 'DELETE',
            url: '/api/POS/pagesvs/' + $scope.data.tableData[i].pk + '/'
          }).
...
app.controller("businessManagement.ecommerce.pages.form", function($scope, $state, $users, $stateParams, $http, Flash, $uibModal, $rootScope) {
  ...
  $scope.resetForm = function(){
    if ($scope.mode == 'new') {
      $scope.form = {title:'',pageurl:'',body:'',topLevelMenu:false,bottomMenu:false,options:'offers'}
    }else {
      $scope.form.title = ''
      $scope.form.pageurl = ''
      $scope.form.body = ''
